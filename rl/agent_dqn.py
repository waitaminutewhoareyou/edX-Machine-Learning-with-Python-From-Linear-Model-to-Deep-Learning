...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        # action_index, object_index = np.argmax(q_action), np.argmax(q_object)
        q_action, q_object = model(state_vector)
...
if __name__ == '__main__':
    ...
    converge_peroid = np.argwhere(reward_ls >= 0.5).flatten()[1:] # drop the first element in case it is an outlier

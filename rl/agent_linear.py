...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    optimal_next_q_ls = 0 if terminal else np.max(theta @ next_state_vector)
    theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)]+ ALPHA*(reward + GAMMA*optimal_next_q_ls - q_value)*current_state_vector
...
if __name__ == '__main__':
    ...
    reward_ls = np.mean(epoch_rewards_test, axis=0)
    converge_peroid = np.argwhere(reward_ls >= 0.4).flatten()[1:] # drop the first element in case it is an outlier
    plt.vlines(converge_peroid[0], 0, max(reward_ls), linestyles="dashed")
    print("Convergence happens at", converge_peroid[0])
    print("Average reward is", np.mean(reward_ls[converge_peroid]))

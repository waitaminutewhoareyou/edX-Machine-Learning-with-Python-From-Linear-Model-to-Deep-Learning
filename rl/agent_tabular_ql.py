...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        _, _, action_index, object_index = np.unravel_index(np.argmax(q_func[state_1, state_2, :, :]), q_func.shape)
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    q_func[current_state_1, current_state_2, action_index,
               object_index] = (1-ALPHA)*q_func[current_state_1, current_state_2, action_index, object_index] + ALPHA*(reward +GAMMA * np.max(q_func[next_state_1, next_state_2, :, :])*(1-terminal))
...
if __name__ == '__main__':
    ...
    converge_peroid = np.argwhere(reward_ls >= 0.5).flatten()

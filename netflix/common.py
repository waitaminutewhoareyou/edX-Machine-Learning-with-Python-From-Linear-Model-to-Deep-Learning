...
def plot(X: np.ndarray, mixture: GaussianMixture, post: np.ndarray,
    ...
    plt.axis('equal')
    plt.savefig("C://Users//JI YIHONG//Dropbox//Edx//Machine Learning with Python From Linear Model to Deep Learning//netflix//plot result//"+ title+".png")
    plt.show()
...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    k = len(mixture.var)
    num_param = k*(d+2)-1
    return log_likelihood - 1/2*num_param*np.log(n)
